...
def mapcompare_d500_first_update_new(data_path,dataset,solution,compute_mode,year,up_para):
    ...
    # second try
    for i in [0.001,0.01]: #0.0005,0.001,
        for b in [45]:#,30
            for t in [i*10]:#
                for c in ['knn']:#,'wmm'    
    ...
    df.to_csv('../output_update/'+dataset+'_'+str(year)+'_d500'+'_'+str(up_para)+'_new3.csv', index=False)